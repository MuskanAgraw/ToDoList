...
struct HomeTasksView: View {
    ...
    var body: some View {
        NavigationView {
            ...
            }.navigationBarTitle(Text("ToDo List").font(.largeTitle))
    ...
    func rowBackgroundColor(dueDate: Date) -> Color {
        return dueDate.isEqual(currentDate: self.selectedDate) ? Color.blue: dueDate.isPast(today: self.selectedDate) ? Color.yellow : Color.pink
    }
